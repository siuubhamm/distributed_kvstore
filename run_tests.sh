...
gcloud compute scp ${CLIENT_VM_NAME}:~/${REPO_DIR_NAME}/latency_results/kvstore_latencies.csv . --zone=$ZONE
echo "Results downloaded successfully."


echo "Script finished. Deleting VMs"
gcloud compute instances delete $SERVER_VM_NAME $CLIENT_VM_NAME --zone=$ZONE --quiet || echo "VM deletion failed, they might need manual cleanup."
echo "VMs deleted."